<!-- ======================= BANNER ======================= -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0e75b6,100:1a1a2e&height=220&section=header&text=Ranit%20Saha&fontSize=48&fontColor=ffffff&animation=fadeIn" />
</p>

<h3 align="center">Software Engineer • Backend & Distributed Systems • Java & Python</h3>

<p align="center">
  <a href="https://github.com/Ranit-cpu">
    <img src="https://komarev.com/ghpvc/?username=Ranit-cpu&style=flat&color=0e75b6"/>
  </a>
  <a href="https://linkedin.com/in/ranit-saha-b58984225">
    <img src="https://img.shields.io/badge/LinkedIn-Ranit%20Saha-blue?logo=linkedin&style=flat"/>
  </a>
  <a href="https://ranitdev.vercel.app">
    <img src="https://img.shields.io/badge/Portfolio-Visit-black?logo=vercel&style=flat"/>
  </a>
  <a href="mailto:ranits003@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-red?logo=gmail&style=flat"/>
  </a>
</p>

---

## 👋 About Me

<img align="right" width="250" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif"/>

Detail-oriented **Software Engineer** with strong foundations in  
**distributed systems, scalable backend development, and cloud architectures**.

- 💻 Backend-first engineer with full-stack exposure  
- ☁️ AWS + Docker + CI/CD practitioner  
- 🧠 Strong CS fundamentals (DSA, OOP, OS, DBMS)  
- 🚀 Own projects end-to-end, from design to production  

---

## 🛠️ Tech Stack

### 💻 Languages
<p>
  <img src="https://skillicons.dev/icons?i=java,python,cpp,javascript" />
</p>

### ⚙️ Frameworks
<p>
  <img src="https://skillicons.dev/icons?i=spring,fastapi,nodejs,react" />
</p>

### 🗄️ Databases
<p>
  <img src="https://skillicons.dev/icons?i=mysql,mongodb,sqlite" />
</p>

### ☁️ Cloud & DevOps
<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,linux,githubactions" />
</p>

### 🧰 Tools
<p>
  <img src="https://skillicons.dev/icons?i=git,github,postman,maven,vscode,idea" />
</p>

---

## 💼 Experience

### 🧑‍💻 Java Developer Intern — **Techplement** *(Remote)*  
📅 **Jun 2025 – Jul 2025**

🔐 **Desktop Password Manager (JavaFX + AES + SQLite)**

- Built secure JavaFX app using **AES encryption** & **MVC architecture**
- Implemented encrypted **JDBC CRUD operations**
- ⚡ Reduced data fetch latency by **40%**
- 👥 Led a **4-member Agile team** (scrum, code reviews)

---

## 🚀 Featured Projects

### 🟦 Technothon Hackathon Platform
<p>
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi"/>
  <img src="https://img.shields.io/badge/React-20232A?style=flat&logo=react"/>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws"/>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker"/>
</p>

- Async **FastAPI** backend with idea submission  
- Hybrid DB (**MySQL + MongoDB + SQLite**) → 🚀 **45% faster queries**  
- Dockerized & deployed on **AWS EC2**  
- CI/CD reduced deployment time by **60%**

🔗 https://technothontiu.com/

---

### 🟩 Employee Management System
<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java"/>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql"/>
</p>

- Multi-tier Java application with authentication  
- Optimized SQL + indexing → **40% faster data loading**  
- Clean **OOP + MVC** architecture  

---

## 🏆 Achievements

🏅 **Winner — Technothon IoT Exposition 2025** *(Best Presentation)*  
🏅 **Winner — IBM Expert Labs National Hackathon 2025**

---

## 📊 GitHub Analytics

## 📊 GitHub Analytics

<p align="center">
  <img
    src="https://github-readme-stats.vercel.app/api?username=Ranit-cpu&show_icons=true&include_all_commits=true&count_private=true&hide_border=true&theme=tokyonight"
    height="170"
    alt="Ranit's GitHub statistics"
  />
  <img
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ranit-cpu&layout=compact&langs_count=8&hide_border=true&theme=tokyonight"
    height="170"
    alt="Ranit's most used languages"
  />
</p>

<p align="center">
  <img
    src="https://streak-stats.demolab.com/?user=Ranit-cpu&theme=tokyonight&hide_border=true"
    height="170"
    alt="Ranit's GitHub streak"
  />
</p>


---

## 🌐 Connect With Me

<p>
  <a href="https://www.linkedin.com/in/ranit-saha-b58984225/" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" width="44"/>
  </a>
  &nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://www.instagram.com/_.r.a.n.i.t._003/" target="_blank">
    <img src="https://skillicons.dev/icons?i=instagram" width="44"/>
  </a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,100:0e75b6&height=120&section=footer"/>
</p>
